...
const Roadmaps = () => (
  <div className="min-h-screen bg-[#0f172a] text-white flex flex-col items-center px-2 pb-16" style={{ fontFamily: 'Gilroy, Inter, Segoe UI, Arial, sans-serif' }}>
    {/* Hero Section */}
    <section className="w-full flex flex-col items-center justify-center pt-14 pb-10 px-2">
      <h1 className="text-4xl md:text-5xl font-bold text-center bg-gradient-to-r from-purple-400 to-pink-400 bg-clip-text text-transparent mb-3">Explore Developer Roadmaps</h1>
      <p className="text-lg md:text-xl text-gray-100 text-center max-w-2xl mb-2">Structured learning paths for every ambition.</p>
      <p className="text-base md:text-lg text-gray-300 text-center max-w-xl italic mb-6">Resources on CampusVault help you discover, plan, and master your journey—one roadmap at a time.</p>
    </section>
    ...
    <div className="w-full max-w-4xl grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5">
      {ROADMAPS.map((roadmap) => (
        <a
          ...
          className="relative bg-[#1e293b] border border-[#334155] rounded-xl px-6 py-6 flex items-center justify-between text-lg font-medium hover:bg-[#334155] transition group shadow"
        >
          <span>{roadmap.label}</span>
          {roadmap.new && (
            <span className="ml-2 px-2 py-0.5 text-xs rounded-full bg-purple-600 text-white font-bold animate-pulse">New</span>
          )}
          <span className="absolute top-2 right-2 text-gray-400 group-hover:text-white">
